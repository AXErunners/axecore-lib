# Contributing to axecore-lib

We're working hard to make *axecore-lib* the most powerful JavaScript library for working with Axe. Our goal is to have *axecore-lib* be a library that can be used by anyone interested in Axe, and to level expertise differences with great design and documentation.

## Community

If there are any questions, etc., please feel to ask in one of the community channels:

- https://www.axe.org/forum (Support Forum)
...
These are some global design goals in axecore-lib that any change must adhere.
...
When possible, axecore-lib objects should have standard methods on an instance prototype:
...
`JSON.stringify` and `JSON.parse` are expected to be handled outside of the scope of axecore-lib methods. For example, calling `JSON.stringify` on an axecore-lib object will behave as expected and call `transaction.toJSON()` and then stringify it:
...
#### T3 - Require 'axecore' and Look up Classes from There

This helps to make tests more useful as examples, and more independent of where they are placed. This also helps prevent forgetting to include all submodules in the axecore object.

DO:
```javascript
var axecore = require('../');
var PublicKey = axecore.PublicKey;
```
DON'T:
```javascript
...
```sh
git remote add axerunners git@github.com:axerunners/axecore-lib.git
git pull --rebase axerunners master
```
...
```
Finally go to [github.com/axerunners/axecore-lib](https://github.com/axerunners/axecore-lib) in your web browser and issue a new pull request.

Main contributors will review your code and possibly ask for changes before your code is pulled in to the main repository.  We'll check that all tests pass, review the coding style, and check for general code correctness. If everything is OK, we'll merge your pull request and your code will be part of axecore-lib.

If you have any questions feel free to post them to
[github.com/axerunners/axecore-lib/issues](https://github.com/axerunners/axecore-lib/issues).
